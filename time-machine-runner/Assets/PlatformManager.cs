...
public class PlatformManager : MonoBehaviour {
	...
	void Update () {
		if (timeTraveller.transform.position.x > (floors [1].transform.position.x +  10)) {
			GameObject tmp = floors[0];
			floors.RemoveAt(0);
			tmp.transform.position = new Vector3(floors[2].transform.position.x + 20, tmp.transform.position.y);
			floors.Add(tmp);
		}
	}
